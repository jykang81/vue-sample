<template>
  <div class="search">
    <header class="app_header search_page">
      <div class="header">
        ...
        <div class="search_wrap">
          <div class="search_inner">
            <span v-if="tvTableFlag" id="tvTagLabel" class="tag_label" :class="setTvTableClass(tvTableFlag)">
              TV쇼핑
              <button
                type="button"
                class="btn_delete"
                @click="tvTagLabelHide()"
              >
                <span>삭제</span>
              </button>
            </span>
      ...
      <ul v-if="isKeyPressing && relateSearchWordCallbackFlag" class="auto_complete">
        <li
          ...
          :key="`${index}`"
          @click="keywordClicked(value.keyword, 'autoKeyword', index)"
        >
    ...
    <div id="searchContainer" class="app_container">
      ...
      >
        <swiper-slide @touchmove="$refs.searchKeyword.blur()">
          <div class="search_word" @touchmove="$refs.searchKeyword.blur()">
            <h3>최근 검색어</h3>
            <ul v-if="hasLatelySearchWord()" @touchmove="$refs.searchKeyword.blur()">
              <li
        ...
        <swiper-slide @touchmove="$refs.searchKeyword.blur()">
          <div class="search_word" @touchmove="$refs.searchKeyword.blur()">
            ...
            <ol
              ...
              v-if="isPopularSearchData()" @touchmove="$refs.searchKeyword.blur()"
            >
              <li
...
export default {
  ...
  created () {
    if (isOsApp()) { // APP
      ...
      } else if (this.tvShopFlag === 'false') { // Shop+ 검색 향후 구현예정임
        this.tvTableFlag = true
      } else {
  ...
  mounted () {
    ...
    if (isOsApp()) { // APP
      if (document.querySelector('#searchContainer') && !document.querySelector('.full_layer')) {
        document.querySelector('#searchContainer').classList.remove('container')
      }
    ...
    this.$nextTick(() => this.$refs.searchKeyword.click())
  },
  destroyed () {
    if (this.liveTalkClassChecker) {
  ...
  methods: {
    ...
    setSearchWordInit () {
      ...
      this.$refs.searchKeyword.value = this.searchKeyword || ''
    },
    /**
     * 검색 input focus 셋팅.
     */
    ...
    searchSubmit () {
      ...
      const trimSearchKeyword = searchKeyword.replace(/\s/g, '') // 검색어 공백 제거.
        ...
        if (isNull(data) || data.msg.root === undefined) {
          ...
          if (!isNull(data.msg.root._GNB_PROMOTION_TEXT_MOB)) {
            ...
            if (voList !== undefined && voList.length > 0) {
              ...
              console.log(`voList[val].marketingText:${voList[val].marketingText}`) // 프로모션 텍스트 데이터 확인후 데이터 셋팅해야함.
            }
          }
        }
      } catch (e) {
     ...
     * @param {String} allKeyword (필수) 자동완성 리스트
    ...
    getOneHourTime () {
      ...
      const returnString = `${year}.${month}.${date} ${hour}:${minute} 업데이트`
      return returnString
    }
  }
}

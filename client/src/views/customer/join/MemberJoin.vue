<template>
  <div class="member_join">
    ...
    <ns-password
      ...
      @focus="passPress(false, 'I')"
      @input="passPress(false, 'I')"
      @blur="passPress(true)"
    />
    <!-- 이름 default -->
    <ns-input
...
export default {
  ...
  beforeRouteLeave (to, from, next) {
    ...
    if (to.name === 'memberComplete' || to.name === 'checkId' || to.name === 'memberLogin') {
      ...
      const okCallback = () => {
        // 이전 라우터로 이동
        if (to.name === 'mypageBenefit' || to.name === 'mypageMain' || to.name === 'myInfoAdmin') {
          const memberInfo = localStorageUtil.get('tmpFacebookMemberInfo')
          localStorageUtil.del('tmpFacebookMemberInfo')
  ...
  data () {
    return {
      ...
      emailParams: {
        ...
        isSuccess: false,
        isError: false,
        errorMessage: MEMBER_CONST.MEMBER_TEXT.EMAIL02
      },
      passwordParams: {
        value: '',
        readonly: false,
  ...
  created () {
    if (loginUtil.isLoggedIn()) {
      ...
      if (!isNull(this.$route.params.email)) {
        this.emailParams.value = this.$route.params.email
        this.checkJoinId()
      }

      if (!isNull(this.$route.params.userName)) {
        this.userNameParams.value = this.$route.params.userName
        this.checkUserName()
      }
    }
  },
  ...
  methods: {
     ...
     * 암호 Key Down이벤트
     *
     * @param {boolean} inputType blur여부
     * @param {string} type 이벤트타입
     * @returns {boolean|void}
     */
    passPress (inputType, type = 'B') {
      this.passwordParams.value = this.passwordParams.value.replace(/ /gi, '')

      if (isNull(this.passwordParams.value.trim())) {
        this.passwordParams.isError = true
        this.passwordParams.errorMessage = MEMBER_CONST.MEMBER_TEXT.PASS02
        return false
      }
      const validPassword = validatePassword(this.passwordParams.value.trim())
      ...
      if (this.pwdGradeCode === '00') {
        this.passwordParams.isSuccess = true
        this.passwordParams.isError = false
        this.isPasswordCheck = true
        this.passwordParams.errorMessage = this.pwdMsg
      } else {
        this.passwordParams.isSuccess = false
        this.passwordParams.isError = true
    ...
    async saveJoin () {
      ...
      const validateInputs = async () => {
        ...
        if (this.passwordParams.isError) {
          this.passPress(true)
        }
      ...
      const checkSuccessState = () => {
        if (this.emailParams.isSuccess && this.passwordParams.isSuccess && this.userNameParams.isSuccess && this.userPhoneParams.isSuccess) {
          return true
        }
      ...
      if (isAllSuccess) {
        ...
        const invoke = {
          ...
          firstName: name,
          lastName: name,
          logonPassword: this.passwordParams.value,
          logonPasswordVerify: this.passwordParams.value,
          affilEntCd: 110,
          entEmail: id,
        ...
        const successHandling = async response => {
          ...
          if (code === 'Y' || code === 'y') {
            ...
            // 회원가입 후 자동 로그인처리
            loginUtil.setUserInfo('userPass', this.passwordParams.value.trim())
            ...
            loginUtil.setUserInfo('logonPassword', this.passwordParams.value)

            // 구글 스마트락 로그인 저장 시작 및 자동로그인 정보 저장
            if (isOsApp()) {
              if (viewType() === 'android' && !isNull(this.passwordParams.value)) {
                nativeUtil.saveCredential(response.msg.memberInfo.logonId, this.passwordParams.value)
              }
              localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_LOGIN_ID, cipherUtil.encryptValue(response.msg.memberInfo.logonId))
              localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_LOGIN_PW, cipherUtil.encryptValue(this.passwordParams.value.trim()))
              localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_ENT_FLAG, '')
              localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_ENT_USER_ID, '')
              localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_ENT_REFRESH_TOKEN, '')
              // localStorageUtil.set(NATIVE_CONST.STORAGE_KEY.AUTO_ENT_USER_INFO, '')
            ...
            const goingInvoke = {
              ...
              params: {
                ...
                userPass: this.passwordParams.value.trim(),
                userId: response.msg.memberInfo.userId
              }
            }

            param.logonId = response.msg.memberInfo.logonId
        ...
        // TODO : 비밀번호 조합 검증 API 호출
        // CurrentView.event.call_ValidatePassword(callbackFunc);
      } else {
        const isSuccessArray = [
          ...
          {
            stat: this.passwordParams.isSuccess,
            inputName: 'pass'
          },
          {
        ...
        for (let i = 0; i < isSuccessArray.length; i++) {
          ...
          if (!tmpMap.stat) {
            ...
            } else if (tmpMap.inputName === 'pass') {
              this.passwordParams.isError = true

              if (this.passwordParams.errorMessage === '') {
                this.passwordParams.errorMessage = MEMBER_CONST.MEMBER_TEXT.PASS02
              }
      ...
      if (this.isSendCertNumber === false) {
        ...
        const successHandling = response => {
          ...
          if (code === 'A') {
            ...
            // const additionalErrorMessage = messageList.slice(1).join('<br>')
            // this.userPhoneParams.additionalErrorMessage = additionalErrorMessage
    ...
    /**
     * 인증번호 전송 후처리
     * @param {import('@components/common/PhoneAuth.vue').resultObject} resultObject
     */
...
<style lang="scss">
  @import "~@/assets/styles/views/customer/join/MemberJoin";
</style>

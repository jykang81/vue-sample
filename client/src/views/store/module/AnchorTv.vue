<template>
  <!-- 앵커: TV -->
  <section class="anchor_tv">
    <!-- 컨텐츠 영역 -->
    <section name="_EZ_HOME_ANCHOR2A_ESPOT_CNTNTPROM">
      <div
        v-if="A2contents"
        class="category_top"
        aria-label="배너"
        @click="bannerCommonClickEvent(
          A2contents.clickTarget,
          A2contents.contentsId,
          A2contents.clickCode,
          A2contents.espotId,
          A2contents.mdUrl,
          null,
          '메인_TV추천_배너',
          isNull(A2contents.marketingText) ? A2contents.contentName : A2contents.marketingText
        )"
      >
        <figure>
          <ns-img
            :src="A2contents.imgUrl"
            :alt="A2contents.marketingText"
            :product-number="A2contents.clickTarget === 'Product' ? A2contents.clickCode : null"
            type="WIDE"
            replace="SQUARE"
          />
        </figure>
        <p class="title_main">
          {{ getHtmlDecode(A2contents.mainTitle) }}
        </p>
        <p class="title_sub">
          {{ getHtmlDecode(A2contents.subTitle) }}
        </p>
      </div>
    </section>
    <!-- // 컨텐츠 영역 -->
    ...
    <!-- 상품 영역 -->
    <section name="_EZ_HOME_ANCHOR2B_ESPOT_PRDWIDE">
      <ul v-if="B2products && B2products.length" class="product_wide">
        <li
          v-for="(value, index) in B2products"
          :key="`root${index}`"
        ...
        >
          <template v-if="isNotNullProductDetail(value)">
            <router-link
              to=""
              event=""
              @click.native="productDetailLogging(value.itncatentrynm, { name: 'productDetail', params: { number: value.partnumber, clksrc: '메인_TV추천_상품' }})"
            >
              <figure>
                <ns-img
              ...
              <div class="price_count">
                ...
                />
                <p v-show="!isOrdQtyHide(B2productsExtend) && Number(value.orderQty) > 0" class="count">
            ...
            <div class="benefit_button">
              ...
              <ul v-else-if="hasBuyOrCartButton(B2productsExtend)" class="benefit" />
              <div
                v-if="hasBuyOrCartButton(B2productsExtend) === 'ORDER'"
                class="button_box"
              >
                <button
              ...
              <div
                v-else-if="hasBuyOrCartButton(B2productsExtend) === 'BASKET'"
                class="button_box"
              >
                <button
                  type="button"
            ...
            <p v-if="!hasBuyOrCartButton(B2productsExtend) && !isNull(value.onAirTime)" class="live_time">
    ...
    <!-- 더보기 영역 -->
    <!-- <div v-if="B2productsExtend && checkMoreView(B2productsExtend)" class="btn_total_view">
      <button
        type="button"
        @click="bannerCommonClickEvent(
          B2productsExtend.titleContent.clickTarget,
          B2productsExtend.titleContent.contentsId,
          B2productsExtend.titleContent.clickCode,
          B2productsExtend.titleContent.espotId,
          B2productsExtend.titleContent.mdUrl
        )"
      >
        {{ getHtmlDecode(B2productsExtend.titleContent.mainTitle) }}
        <i />
      </button>
    </div> -->
    <!-- // 더보기 영역 -->
...
export default {
  ...
  data () {
    return {
      ...
      A2contents: this.espotData.espotList._EZ_HOME_ANCHOR2A_ESPOT_CNTNTPROM[0],
      A2contentsExtend: this.espotData.espotExtendList._EZ_HOME_ANCHOR2A_ESPOT_CNTNTPROM,
      B2products: this.espotData.espotList._EZ_HOME_ANCHOR2B_ESPOT_PRDWIDE,
      B2productsExtend: this.espotData.espotExtendList._EZ_HOME_ANCHOR2B_ESPOT_PRDWIDE
    }
  },
  methods: {
    getCutBytes,
...
<style lang="scss">
    @import "~@/assets/styles/views/store/module/AnchorTv";
</style>

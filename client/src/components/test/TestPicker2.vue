<template>
  <div class="vscroller__body">
    <ul ref="ul" :style="{ webkitTransition: '-webkit-transform ' + transitionDuration / 1000 + 's ease-out', transition: 'transform ' + transitionDuration / 1000 + 's ease-out', webkitTransform: 'translate3d(0px, ' + currentTranslatedY + 'px, 0px)', transform: 'translate3d(0px, ' + currentTranslatedY + 'px, 0px)' }">
      <li v-for="item in itemList" :key="item.id" :data-val="item.value" :class="item.selected ? 'vselected' : ''">
        {{ item.name }}
      </li>
    </ul>
  </div>

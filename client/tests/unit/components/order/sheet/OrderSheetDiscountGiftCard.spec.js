...
import OrderSheetDiscountGiftCard from '@/components/order/sheet/OrderSheetDiscountGiftCard'

describe('OrderSheetDiscountGiftCard', () => {
  let localVue
  let options
  ...
  describe('OrderSheetDiscountGiftCard 테스트', () => {
    let wrapper
    beforeEach(async () => {
      wrapper = mount(OrderSheetDiscountGiftCard, {
        propsData: {
          param: {
    ...
    it('NS상품권(methods())- 확인 (onclickOk)', async () => {
      wrapper.vm.giftCardNo = '12345678'
      ...
      wrapper.vm.giftCardNo = '1234567890123456'
      wrapper.vm.onclickOk()
      assert.isTrue(true)
    })
    it('NS상품권(methods())- 취소 (onclickCancel)', async () => {
      // modalUtil.show('order/sheet/OrderSheetDiscountGiftCard', { }, () => {})
      // wrapper.vm.onclickCancel()
      assert.isTrue(true)
    })
    it('NS상품권(methods())- 입력 (key)', async () => {
      e.keyCode = 9

...
import OrderSheetDiscountOkCashbag from '@/components/order/sheet/OrderSheetDiscountOkCashbag'

describe('OrderSheetDiscountOkCashbag', () => {
  let localVue
  let options
  ...
  describe('OrderSheetDiscountOkCashbag 테스트', () => {
    let wrapper
    beforeEach(async () => {
      wrapper = mount(OrderSheetDiscountOkCashbag, {
        propsData: {
          param: {
    ...
    it('OK캐쉬백(methods())- 확인 (onclickOk)', async () => {
      ...
      wrapper.vm.identity = '1234567890123456'
      wrapper.vm.passwd = ''
      assert.isFalse(wrapper.vm.onclickOk())

      wrapper.vm.passwd = '1234567890123456'
      wrapper.vm.onclickOk()
      assert.isTrue(true)
    })
    it('OK캐쉬백(methods())- 취소 (onclickOk)', async () => {
      // modalUtil.show('order/sheet/OrderSheetDiscountOkCashbag', { }, () => {})
      // wrapper.vm.onclickCancel()
      assert.isTrue(true)
    })
    it('OK캐쉬백(methods())- 입력 (key)', async () => {
      e.keyCode = 9

...
import ParticipationWinner from '@/views/event/ParticipationWinner' // 테스트 할 대상 컴포넌트
import { nsaJaxMTimesEventCmdUrl, winnerListRes, memberInfo } from '@unit/views/event/mock/participation'

// 필수 테스트 할 대상 파일명을 넣음
describe('ParticipationWinner.vue에 대한 테스트 케이스', () => {
  let localVue = null
  let options = null
  let mock
  ...
  describe('method : 테스트 코드 커버리지 용', () => {
    it('getNoticeListMobile Function Call', async () => {
      ...
      // 테스트 대상 컴포넌 명 입력
      const wrapper = shallowMount(ParticipationWinner, options)
      const vm = wrapper.vm
      ...
      try {
        ...
        vm.getNoticeListMobile()

        await flushPromises()

        await vm.$nextTick()

        const pageIdx = vm.pageId
    ...
    it('chageNoticeListMobile Function Call', async () => {
      mock
        .onPost(nsaJaxMTimesEventCmdUrl)
        .reply(200, winnerListRes)

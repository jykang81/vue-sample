...
import passwordChange3th from '@/views/customer/login/PasswordChange3th'
import Vue from 'vue'
import nsaxios from '@frameworks/axiosUtil'

describe('PasswordChange3th', () => {
  let mock
  ...
  it('sets the correct default data', () => {
    assert.equal(typeof passwordChange3th.data, 'function')
    const defaultData = passwordChange3th.data()
    assert.equal(defaultData.inputNewPwdParams.isSuccess, false)
  })

  describe('변경암호 처리', () => {
    it('입력값이 정책에 맞지 않는 경우', () => {
      const wrapper = mount(passwordChange3th, options)
